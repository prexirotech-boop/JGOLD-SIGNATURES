...
export default function AdminCertificates() {
  ...
  const loadData = async () => {
    ...
    const { data, error } = await supabase
      .from('certificates')
      .select('id, certificate_number, issued_at, is_valid, profiles(full_name, email), courses(id, products(title))')
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24, fontFamily: 'var(--font)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
        {[
          ...
          <div key={s.label} style={{ background: '#fff', border: '1px solid #e3e8ee', borderRadius: 8, padding: '18px 20px', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 600, color: '#697386', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{s.label}</span>
            </div>
      ...
      <div style={{ background: '#fff', border: '1px solid #e3e8ee', borderRadius: 8, overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
        <div style={{ padding: '12px 16px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ position: 'relative', flex: 1, maxWidth: 320 }}>
            <svg viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="2" style={{ width: 14, height: 14, position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }}>
            ...
            <input
              ...
              style={{ width: '100%', padding: '7px 12px 7px 40px', border: '1px solid #e2e8f0', borderRadius: 6, fontSize: 12.5, outline: 'none', boxSizing: 'border-box', background: '#f8fafc' }}
            />
          </div>
